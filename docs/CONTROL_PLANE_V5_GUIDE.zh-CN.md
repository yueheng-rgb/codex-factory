...
下面的配置适合多数复杂项目：启用独立上下文和多 Agent，不启用外部搜索，并允许最多 4 个原生工作 Agent（`max-threads=4` 不包含主 Agent）。
...
目标项目中的 V5 状态以 `.codex-factory/` 为边界，包括配置、Context Space、运行账本、Context Packet 和本地 secrets 文件。安装器还会安装受管 Agent profile、角色/领域 Skill 与 `.codex/hooks.json` 生命周期 Hook。
...
- 第一次打开项目或 Hook 内容变化后，在 Codex 中用 `/hooks` 完成宿主要求的审阅和信任；Factory 不绕过此安全边界。
...
5. 由 Pre/PostToolUse Hook 校验权威 assignment，并直接绑定 Codex 原生返回的 Agent ID、昵称和生命周期。
...
| 类型 | 角色 | 生命周期 | 主要边界 |
| --- | --- | --- | --- |
| 常驻配置 | Router | 配置长期存在；每个 assignment 新实例 | 只做路由和任务波次，不实现 |
| 常驻配置 | Librarian | 配置长期存在；每个 assignment 新实例 | 只取相关 Skill/知识，标记过期材料 |
| 常驻配置 | Verifier | 配置长期存在；每个 assignment 新实例 | 独立、只读、失败关闭，不替实现者修复 |
| 常驻配置 | Drift Auditor | 配置长期存在；每个 assignment 新实例 | 比对需求/架构/权限/文件漂移，不修复 |
...
“常驻”是配置和职责长期存在，不是始终占用线程的后台守护进程。Factory 对每个 assignment 都以 `fork_turns=none` 生成全新执行实例，不复用旧 Agent 对话；临时 Agent 也只在必要时创建。
...
- 初始化会安装 8 个角色协议 Skill 和 9 个任务级领域 Skill；领域 Skill 按 capability 路由，避免把无关专业知识全部塞入上下文。迁移目录后需重新运行 `init` 刷新绝对 Skill 路径和 Hook 命令。
...
当前规划器不复用常驻 Agent 对话，也不为新 assignment 生成 `followup_task`。项目 Hook 会从真实 Host tool response 自动登记派发；`agent lifecycle`、`handoff record` 与 `verify` 是底层审计协议，普通用户通常只查看 `agent status` 和最终验证输出，不应手工捏造回执、交接或 PASS。
...
`required_capabilities` 可显式选择专业路由；也可用 `profile_id` 固定到允许且具备这些能力的 Factory profile。`write_scope` 使用项目相对目录或文件，不要用含糊的“整个项目”。规划器会为写任务补充独立验证任务，并拒绝同一并行波次中相互覆盖的写入范围。
...
初始化会安装 Router、Librarian、Verifier、Drift Auditor、Researcher、Implementer、Tester 和 Integrator 八个角色协议 Skill，并安装 app 分类、产品架构、防过度设计、前端、后端、数据库、认证安全、移动端和 Web 测试九个领域 Skill。领域 Skill 根据 assignment capability 路由，不会静态灌入每个 Agent。Skill 负责工作协议，知识库负责项目事实；两者不能互相冒充来源证据。移动项目目录后重新运行 `factoryctl init --project <新路径>`，刷新 Agent TOML 与 Hook 中的绝对路径。
...
5. 在确认 `.codex/hooks.json` 的 `description` 为 Factory managed hooks 后再移除该文件；
6. 确认没有混入用户自己的规则或 profile 后，再删除 `.codex-factory/`；
7. 如果使用过 `npm link`，在 CLI 包目录运行 `npm unlink`。

不要为了卸载而整文件删除用户已有的 `AGENTS.md`、`.codex/config.toml` 或 `.codex/agents/`。若 managed marker 不完整，先停止并人工审阅 diff。

### 10.5 从旧 V5 Preview 升级

在 Factory 仓库更新并重新构建 `factoryctl` 后，对每个已有项目重复运行同一组开关即可做幂等升级：
...
升级会刷新 Factory 自有的 Agent、角色 Skill、9 个领域 Skill、Hook 和线程配置，不覆盖无法证明为 Factory 所有的同名文件。旧 run 与记忆账本保留；旧的 idle 常驻实例不会再占用新 wave 的线程，也不会被复用。由于 Hook 内容发生变化，下一次打开项目时需在 Codex `/hooks` 中重新审阅当前哈希。
...
| Hook 显示未信任或没有回执 | Codex `/hooks`、项目是否受信任、`.codex/hooks.json` 是否为最新 | 审阅并信任当前 Hook；不要用 `--dangerously-bypass-hook-trust` 作为日常安装方案 |
