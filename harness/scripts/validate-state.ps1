# Validate State 鈥?READ-ONLY Gate 鈥?Phase 6B-R3
...
# --- 11. Interface Contract Lock ---
$contractLockPath = Join-Path $RunDir "interface-contract.lock.json"
if (Test-Path $contractLockPath) {
    try { $contractLock = Get-Content $contractLockPath -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $contractLock = $null }
    if ($contractLock) {
        [void]$passes.Add("Interface contract lock found")
        if ($contractLock.locked -eq $true) { [void]$passes.Add("Interface contract locked = true") }
        else { [void]$errors.Add("INTERFACE_CONTRACT_NOT_LOCKED"); $exitCode = 1 }
    }
} else {
    [void]$warnings.Add("No interface contract lock file")
}

# --- 12. Worker Interface Manifests + Drift + Honesty + Integration ---
$manifestsDir = Join-Path $RunDir "worker-interface-manifests"
$sourceManifestsDir = Join-Path $RunDir "source-derived-interface-manifests"
$reportsDir = Join-Path $RunDir "reports"

# 12a: Drift report
$driftReportPath = Join-Path $reportsDir "interface-drift-report.json"
if (Test-Path $driftReportPath) {
    try { $driftReport = Get-Content $driftReportPath -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $driftReport = $null }
    if ($driftReport) {
        if ($driftReport.verdict -eq "PASS" -and $driftReport.driftCount -eq 0) {
            [void]$passes.Add("Interface drift report: PASS")
            [void]$passes.Add("Interface drift count: 0")
        } else {
            [void]$errors.Add("INTERFACE_DRIFT_FAIL"); $exitCode = 1
        }
    }
} else {
    [void]$warnings.Add("No interface drift report")
}

# 12b: Integration gate report
$gateReportPath = Join-Path $reportsDir "integration-gate-report.json"
if (Test-Path $gateReportPath) {
    try { $gateReport = Get-Content $gateReportPath -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $gateReport = $null }
    if ($gateReport) {
        if ($gateReport.verdict -eq "PASS") { [void]$passes.Add("Integration gate: PASS") }
        else { [void]$errors.Add("INTEGRATION_GATE_FAIL"); $exitCode = 1 }
        if ($gateReport.dependencies.interfaceDriftReport -eq "PASS") { [void]$passes.Add("Integration gate depends on drift report") }
    }
} else {
    [void]$warnings.Add("No integration gate report")
}

# 12c: Worker interface manifests + source-derived + honesty
if (Test-Path $manifestsDir) {
    $manifests = @(Get-ChildItem $manifestsDir -File -Filter "*.json" | Sort-Object Name)
    [void]$passes.Add("Worker interface manifests: $($manifests.Count) files")
    if ($manifests.Count -eq 0) { [void]$errors.Add("NO_WORKER_INTERFACE_MANIFESTS"); $exitCode = 1 }
} else {
    [void]$warnings.Add("No worker interface manifests directory")
}

if (Test-Path $sourceManifestsDir) {
    $sourceManifests = @(Get-ChildItem $sourceManifestsDir -File -Filter "*.json" | Sort-Object Name)
    if ($sourceManifests.Count -gt 0) {
        [void]$passes.Add("Source-derived manifests found: $($sourceManifests.Count) files")
        foreach ($smf in $sourceManifests) {
            $workerId = $smf.BaseName
            $wmPath = Join-Path $manifestsDir "$workerId-interface-manifest.json"
            if (Test-Path $wmPath) {
                $honestyReportPath = Join-Path $reportsDir "manifest-honesty-report-$workerId.json"
                if (Test-Path $honestyReportPath) {
                    try { $hr = Get-Content $honestyReportPath -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $hr = $null }
                    if ($hr -and $hr.verdict -eq "PASS") {
                        [void]$passes.Add("Manifest honesty: $workerId PASS")
                        $mismatchCount = if ($hr.mismatches) { $hr.mismatches.Count } else { 0 }
                        [void]$passes.Add("Manifest honesty mismatches: $mismatchCount ($workerId)")
                    } else {
                        [void]$errors.Add("MANIFEST_HONESTY_FAIL: $workerId"); $exitCode = 1
                    }
                } else {
                    [void]$errors.Add("MISSING_MANIFEST_HONESTY_REPORT: $workerId"); $exitCode = 1
                }
            } else {
                [void]$errors.Add("NO_WORKER_MANIFEST_FOR_SOURCE: $workerId"); $exitCode = 1
            }
        }
    }
}

# --- 13. U0-E Parallel Overlap + Workspace Isolation Gate ---
$overlapReport = Join-Path $reportsDir "parallel-overlap-report.json"
$isolationReport = Join-Path $reportsDir "workspace-isolation-report.json"
if ((Test-Path $overlapReport) -and (Test-Path $isolationReport)) {
    try { $overlap = Get-Content $overlapReport -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $overlap = $null }
    try { $isolation = Get-Content $isolationReport -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $isolation = $null }
    if ($overlap) {
        if ($overlap.verdict -eq "PASS") { [void]$passes.Add("Parallel overlap: PASS") }
        else { [void]$errors.Add("PARALLEL_OVERLAP_FAIL"); $exitCode = 1 }
        if ($overlap.overlap.exists -eq $true) { [void]$passes.Add("Parallel overlap exists") }
        else { [void]$errors.Add("NO_PARALLEL_OVERLAP"); $exitCode = 1 }
        if ($overlap.overlap.overlapSeconds -gt 0) { [void]$passes.Add("Overlap seconds: $($overlap.overlap.overlapSeconds)") }
        else { [void]$errors.Add("OVERLAP_ZERO_SECONDS"); $exitCode = 1 }
    }
    if ($isolation) {
        if ($isolation.verdict -eq "PASS") { [void]$passes.Add("Workspace isolation: PASS") }
        else { [void]$errors.Add("WORKSPACE_ISOLATION_FAIL"); $exitCode = 1 }
        if ($isolation.ownershipViolation -eq $true) { [void]$errors.Add("OWNERSHIP_VIOLATION"); $exitCode = 1 }
        else { [void]$passes.Add("No ownership violation") }
    }
}

# --- 14. U0-F Rework Loop Gate ---
$reworkRequestsDir = Join-Path $RunDir "rework-requests"
$reworkResolutionsDir = Join-Path $RunDir "rework-resolutions"
if (Test-Path $reworkRequestsDir) {
    $rrs = @(Get-ChildItem $reworkRequestsDir -File -Filter "*.json" | Sort-Object Name)
    if ($rrs.Count -gt 0) {
        [void]$passes.Add("Rework requests found: $($rrs.Count)")
        foreach ($rr in $rrs) {
            try { $rrData = Get-Content $rr.FullName -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $rrData = $null }
            if ($rrData -and $rrData.blocking -eq $true) {
                $resolutionPath = Join-Path $reworkResolutionsDir "$($rr.BaseName)-resolution.json"
                if (Test-Path $resolutionPath) {
                    try { $resData = Get-Content $resolutionPath -Raw -Encoding UTF8 | ConvertFrom-Json } catch { $resData = $null }
                    if ($resData -and $resData.status -eq "resolved") {
                        [void]$passes.Add("Rework $($rr.BaseName): RESOLVED")
                    } else {
                        [void]$errors.Add("REWORK_UNRESOLVED: $($rr.BaseName)")
                        $exitCode = 1
                    }
                } else {
                    [void]$errors.Add("REWORK_NO_RESOLUTION: $($rr.BaseName)")
                    $exitCode = 1
                }
            } else {
                [void]$passes.Add("Rework $($rr.BaseName): non-blocking (info only)")
            }
        }
    }
}
# --- FINAL VERDICT ---
# R2 fix: verdict and exit code derived from actual error count, never from mutable intermediate variable
$verdict = if ($errors.Count -eq 0) { "run_passed" } else { "run_failed" }
